...
def render_md(data, out):
    ...
    w('- Готовый патч показывай, когда причина подтверждена логами; пока не '
      'подтверждена — называй направление фикса с оговоркой об уверенности, но '
      'разбор без предложения не оставляй.\n')
